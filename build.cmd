@echo off
chcp 65001 >nul 2>&1
REM AdeBuddy Build Script (Frontend + Backend + PyInstaller + Installer)
REM Usage: build.cmd [--windowed] [--clean] [--no-frontend] [--no-verify] [--no-installer] [--version 1.0.0]
setlocal enabledelayedexpansion

cd /d "%~dp0"

echo [build] AdeBuddy Build
...
if exist "dist\AdeBuddy\_internal\tools\dist-ui\index.html" (
    echo [build]   OK: _internal\tools\dist-ui\index.html
) else if exist "dist\AdeBuddy\tools\dist-ui\index.html" (
...
if exist "dist\installer\AdeBuddy-Setup-*.exe" (
    for %%f in (dist\installer\AdeBuddy-Setup-*.exe) do (
...
echo [build] ========================================
echo [build]   Output dir:  dist\AdeBuddy\
echo [build]   Executable:  dist\AdeBuddy\AdeBuddy.exe
if "!INSTALLER_FOUND!"=="1" (
    echo [build]   Installer:   dist\installer\AdeBuddy-Setup-*.exe
) else (
    echo [build]   Distribute:  zip dist\AdeBuddy\ or install Inno Setup
)
echo [build]
