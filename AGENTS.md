...
**问题**：界面交互改动直接给单套方案，用户难以对比权衡不同交互形态，上线后才发现体验问题需要回滚。

**规则**：
- 涉及**前端界面变化、交互流程改动**（按钮/菜单/工作流/页面结构/状态切换等）时，
  **必须提供至少三套不同的交互方案**供用户选择，禁止只给一套
- 三套方案需有明显差异（布局/层级/操作路径不同），不能是同一思路的微调
- 每套方案说明：交互形态、改动点、优点、代价
- 用户明确选定其中一套（或要求合并/调整）后，才能进入实现阶段
- **例外**：纯样式微调（颜色/间距/字号）、文案修改、bug 修复不触发本规则
