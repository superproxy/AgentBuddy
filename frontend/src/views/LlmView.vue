        ...
        <template v-if="selectedProvider && envData.llm[selectedProvider]">
          ...
          <div class="px-[18px] py-4 pb-5 grid gap-3.5 overflow-auto">
            ...
            >
              ...
              <!-- 协议级 base_url / api_key（覆盖 _base） -->
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-2.5">
                <div class="flex flex-col gap-1">
                  <label class="text-[11px] font-medium text-ink-700 flex items-center gap-1.5">
                    base_url
                    <span v-if="!envData.llm[selectedProvider][proto].base_url && providerBase?.base_url" class="text-[10px] text-ink-400">继承基础配置</span>
                  </label>
                  <input
                    v-model="envData.llm[selectedProvider][proto].base_url"
                    :placeholder="providerBase?.base_url || 'https://api.example.com/v1'"
                    class="w-full px-2.5 py-2 text-xs border border-ink-300 rounded-lg font-mono"
                    :class="{ 'border-dashed border-ink-300': !envData.llm[selectedProvider][proto].base_url }"
                  />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-[11px] font-medium text-ink-700 flex items-center justify-between">
                    <span class="flex items-center gap-1.5">
                      api_key
                      <span v-if="!envData.llm[selectedProvider][proto].api_key && providerBase?.api_key" class="text-[10px] text-ink-400">继承基础配置</span>
                    </span>
                    <span
                  ...
                  <div class="relative flex items-center">
                    <input
                      ...
                      :placeholder="isEnvRef(providerBase?.api_key) ? '${VAR_NAME}' : (providerBase?.api_key ? '继承基础配置' : 'sk-...')"
                      class="w-full px-2.5 py-2 pr-10 text-xs border border-ink-300 rounded-lg font-mono"
                      :class="{
                        'border-emerald-300 bg-emerald-50/40': isEnvRef(envData.llm[selectedProvider][proto].api_key),
                        'border-dashed border-ink-300': !envData.llm[selectedProvider][proto].api_key
                      }"
                    />
                    <button
                      type="button"
