...
export const useSubagentStore = defineStore('subagent', () => {
  ...
  async function saveSubagent(silent = false) {
    const r = await api<{ ok: boolean; error?: string }>('/api/subagent', {
      method: 'POST', body: JSON.stringify({ data: subagentData }),
    })
    if (!silent) r.ok ? ui.toast('subagent.yaml 已保存') : ui.toast('保存失败: ' + r.error, 'err')
    return r.ok
  }
  function addSubagent() {
    subagentData.subagents.push({ name: '', role: '', desc: '', category: '', prompt: '' })
  }
  function deleteSubagent(idx: number) {
    subagentData.subagents.splice(idx, 1)
  }

  return { subagentData, loadSubagent, saveSubagent, addSubagent, deleteSubagent }
})
