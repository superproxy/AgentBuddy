...
def _resolve_litellm_cmd() -> str:
    ...
    查找顺序：PATH → exe 同级目录 → Python Scripts 目录 → fallback
    """
    import shutil
    # 1. PATH 中的 litellm
    if shutil.which("litellm"):
        return "litellm"
    # 2. exe 同级目录（PyInstaller 打包后 litellm CLI 在此）
    exe_dir = Path(sys.executable).parent
    litellm_exe = exe_dir / ("litellm.exe" if sys.platform == "win32" else "litellm")
    if litellm_exe.exists():
        return _shell_quote(str(litellm_exe))
    # 3. Python Scripts/bin 目录
    py = _get_python()
...
def api_ide_launch():
    ...
    source = (body.get("source") or "").strip()
    extra_args = body.get("extra_args") or None
    if isinstance(extra_args, list):
        extra_args = [str(a) for a in extra_args if str(a).strip()]
        if not extra_args:
            extra_args = None
    else:
        extra_args = None
    print(
        ...
        f"cwd={cwd!r} mode={mode!r} source={source!r} extra_args={extra_args!r}",
        flush=True,
    )
    try:
        if session_id:
            result = launch_ide_resume_session(ide_key, session_id, cwd, mode=mode, source=source)
        else:
            result = launch_ide(ide_key, cwd, mode=mode, source=source, extra_args=extra_args)
...
def api_ide_install():
    ...
    Body: {ide: <IDE key>, mode: "cli" | "app" | "vscode" | "idea" | "acp"}
    Returns: {ok, ide, mode, method, message, cmd, stdout, stderr, url?}
    - cli/app：执行实际安装命令
    - vscode/idea/acp：返回 URL（deep link 或市场页），前端通过 /api/ide/open-url 打开
    """
    body = request.get_json(silent=True) or {}
    ide_key = (body.get("ide") or "").strip()
    ...
    if not ide_key:
        return jsonify({"ok": False, "error": "missing ide"}), 400
    if mode not in ("cli", "app", "vscode", "idea", "acp"):
...
def api_ide_uninstall():
    ...
    Body: {ide: <IDE key>, mode: "cli" | "app" | "vscode" | "idea" | "acp", force?: bool}
